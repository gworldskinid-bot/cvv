...
async def admin_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reply_to and not text:
        return await update.message.reply_text("⚠️ Gunakan `/broadcast <teks>` atau balas (reply) pesan.", parse_mode="Markdown")
        
    users_col = context.bot_data["users_col"]
    users = await users_col.find({}).to_list(length=None)
    msg = await update.message.reply_text(f"⏳ *Memulai Broadcast ke {len(users)} member...*", parse_mode="Markdown")
