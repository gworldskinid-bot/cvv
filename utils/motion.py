...
async def async_post(*args, **kwargs):
    ...
    res = await asyncio.to_thread(requests.post, *args, **kwargs)
    
    url = args[0] if args else kwargs.get('url', 'Unknown URL')
    print(f"[DEBUG POST] URL: {url} | Status: {res.status_code} | Response: {res.text[:100]}...")
...
async def upload_image_crushnow(img_bytes, filename="image.jpg"):
    ...
    try:
        res_up = await asyncio.to_thread(requests.post, url_up, headers=headers, files=files, timeout=20)
        if res_up.status_code == 200:
            res_json = res_up.json()
...
async def get_video_url_ssstik(tiktok_url):
    ...
    # --- LAYER 1: TIKWM (Paling Cepat & Stabil) ---
    ...
    # Jika ketiga server benar-benar down
...
async def submit_motion_task(image_bytes, image_name, tiktok_url, replace_background=False):
    ...
    # MENGGUNAKAN CRUSHNOW API UNTUK UPLOAD GAMBAR
